...
model_save_path = f'longformer/training/train_bi-encoder-margin_mse-{model_name.replace("/", "-")}-batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
data_folder = 'data/msmarco-data-passages'
...
ce_scores_file = os.path.join(data_folder, 'cross-encoder-ms-marco-MiniLM-L-6-v2-scores.pkl')
...
# Script was called via:
#python train_longformer_new.py --train_batch_size 8 --model_name allenai/longformer-base-4096