...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-save-path", default='longformer/')
    parser.add_argument("--mode", default='load')
    parser.add_argument("--data-dir", default='longformer/data')
    parser.add_argument("--save-indicator", default='')
    parser.add_argument("--output-indicator", default='')
    ...
    model.fit(train_objectives=[(train_dataloader, train_loss)],
            epochs=3,
            evaluation_steps=int(0.1*len(train_dataset)),
            use_amp=True,
            scheduler='warmupconstant',                             # linear warmup schedule
            warmup_steps=int(0.1*len(train_dataset)),               # number of warmup steps for learning rate scheduler
            weight_decay=10**-7,                                    # strength of weight decay
            output_path=f'{args.model_save_path}/train_output_{args.output_indicator}',     # Save training output
            save_best_model=True, 
            checkpoint_path=f'{args.model_save_path}/checkpoints_{args.output_indicator}',  # Folder to save checkpoints during training
            checkpoint_save_steps=int(0.1*len(train_dataset)),      # Will save a checkpoint after so many steps 
    )
    model.save('longformer/final_model')
